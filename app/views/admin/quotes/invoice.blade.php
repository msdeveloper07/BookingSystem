<html>
    <body>
        ...
        <?php $totalperson = $quote->number_of_children + $quote->number_of_adults;  ?>
       <tr>
            <td style="width:15%; border-bottom: 1px solid #000;"><strong>{{$totalperson}}</strong></td>
            <td style="width:50%; border-bottom: 1px solid #000;">Confirming your quote with us for the&nbsp;<b>{{$location_to}}</b>&nbsp;from&nbsp;<b>{{$location_from}}</b>&nbsp;on&nbsp;<b>{{ZnUtilities::format_date($quote->date_from,'2')}}</b>&nbsp;for&nbsp;<b>{{$quote->number_of_days}}</b>&nbsp;days.</td>
            <td style="width:15%; border-bottom: 1px solid #000;text-align:right; "><strong>{{$quote->total_cost}}</strong></td>
            <td style="width:20%; border-bottom: 1px solid #000;text-align:right; "><strong>{{$quote->quoted_cost}}</strong></td> 
        </tr>
        ...
        <?php  $items = QuoteFeature::where('quote_id',$quote->quote_id)->get();  ?>
         ...
         <tr>
            <td colspan="4" style="border-bottom: 0.5px solid #000;"><div style="width:100%;"><h4><center>Quote Details</center></h4></div></td>
        </tr>
       ...
       <tr style="width:100%;">
            ...
            <td style="width:10%; border-bottom: 1px solid #000; "><strong>{{ZnUtilities::format_date($quote->date_from,'2')}}</strong></td>
            <td style="width:50%; border-bottom: 1px solid #000;"><strong>{{ZnUtilities::format_date($quote->date_to,'2')}}</strong></td> 
        </tr>
