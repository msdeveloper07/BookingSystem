...
<div class="container" style="margin-top: 40px;">
    ...
    <div class="row">
        ...
        <div class="span8">
            <h2>Quote Title: {{$quote->quote_title}}</h2>
        </div>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$location_from}}</td>
                        <td>{{$location_to}}</td>
                        <td>{{ZnUtilities::format_date($quote->date_from,'2')}}</td>
                        <td>{{ZnUtilities::format_date($quote->date_to,'2')}}</td>

                        <td>{{$quote->number_of_days}}</td>
                        <td>{{$totalperson}}</td>
                        <td>{{$quote->quoted_cost}}</td>
   ...
   <div class="row span8 well invoice-thank">
            ...
            <?php $supplier_parent = QuoteFeature::where('quote_id', $quote->quote_id)->distinct()->lists('supplier_parent_id');  ?>
 ...
 @foreach($supplier_parent as $s)
  <?php $stpid = SupplierType::where('supplier_type_id', $s)->pluck('supplier_type');  ?>
    ...
    <?php  $items = QuoteFeature::where('quote_id', $quote->quote_id)->where('supplier_parent_id',$s)->get(); ?>
    @foreach($items as $i)
