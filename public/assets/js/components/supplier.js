...
$(document).ready(function(){
    ...
    $.getJSON( "/ajax/getsupplierItemInfo/", function( data ) {
     ...
     /*   var numItems = $('#box_body1 .holder_div').length;
       ...
       for(var n=1;n<=numItems;n++){
        $('#supplier_type_id_'+n).change(function(){
    ...
    $.each(data,function(index,value){
        ...
        if(val==value.supplier_type_id){
           $('#supplier_item_id_'+n).empty();
           
           var opt = $('<option>'); 
           opt.val();
            opt.text("Select Item");
            $('#supplier_item_id_'+n).append(opt);
           $.each(value.items,function(key,Value){
             var opt = $('<option>'); 
               opt.val(Value.supplier_item_id);
      ...
      $.each(supplierData,function(k,v){
          if(v.supplier_type_id==value)
          {
              ...
              $.each(v.items,function(m,i){
                  html += '<option value="'+i.supplier_type_item_id+'">'+i.supplier_item_name+'</option>';
              });
